...
void websocketacceptKey(char *clientKey, char *Output)
{
	...
	SHA1Context ctx;
	SHA1Init(&ctx);
	SHA1Update(&ctx, (unsigned char *)clientKey, strlen(clientKey));
	SHA1Final(&sha1HashBin[0], &ctx);
	// wc_Sha sha;
	//  wc_InitSha(&sha);
	// wc_ShaUpdate(&sha, (unsigned char *)clientKey, strlen(clientKey));
	// wc_ShaFinal(&sha, &sha1HashBin[0]);
...
bool websocketnewclient(int socket)
{
	...
	for (i = 0; i < NBCLIENT; i++)
		if (webserverclients[i].socket == socket)
			...
			for (i = 0; i < NBCLIENT; i++)
				if (webserverclients[i].socket == -1)
				{
...
bool iswebsocket(int socket)
{
	int i;
	for (i = 0; i < NBCLIENT; i++)
		if ((webserverclients[i].socket != -1) && (webserverclients[i].socket == socket))
			return true;
	return false;
}
///////////////////////////
...
void websocketparsedata(int socket, char *buf, int len)
{
	...
	if (header.payloadLen > MAXDATA - WEBSOCKETS_MAX_HEADER_SIZE)
	{ // we must be in one buf max for payload
		// disconnect
		return;
	}
