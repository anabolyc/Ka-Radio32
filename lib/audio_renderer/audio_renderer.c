...
static void init_i2s(renderer_config_t *config)
{
	...
	if ((config->output_mode == I2S) || (config->output_mode == I2S_MERUS) || (config->output_mode == SPDIF))
	{
...
static void render_i2s_samples(char *buf, uint32_t buf_len, pcm_format_t *buf_desc)
{
	...
	if (buf_desc->bit_depth == renderer_instance->bit_depth && buf_desc->buffer_format == PCM_INTERLEAVED && buf_desc->num_channels == 2 && renderer_instance->output_mode != DAC_BUILT_IN && renderer_instance->output_mode != PDM)
	{
