...
void setfont(sizefont size)
{
	...
	case text:
		...
		case 240:
			...
			{
			case Cyrillic:
				ucg_SetFont(&ucg, ucg_font_crox5h);
				break;
			case Greek:
		...
		if (lline[i] != NULL)
		{
			...
			else
			{
				...
				if (len > x)
				{
					...
					while ((*(lline[i] + iline[i]) != ' ') && (*(lline[i] + iline[i]) != '-') && (iline[i] != 0))
						iline[i]--;
					if (iline[i] == len)
						iline[i] += (x / ucg_GetStrWidth(&ucg, "MM")); //x/6;
					mline[i] = 1;									   //draw(i);
...
void setColor(int i)
{
	...
	case TITLE2:
	case TITLE21:
		ucg_SetColori(&ucg, 0, 255, 255);
		break;
	case VOLUME:
		ucg_SetColori(&ucg, 200, 200, 255);
		break;
	default:
...
void draw(int i)
{
	...
	case STATIONNAME:
		...
		{
			if (nameNum[0] == 0)
				ucg_DrawString(&ucg, 1, 1, 0, lline[i] + iline[i]);
			else
			{
