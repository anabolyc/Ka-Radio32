	...
	for (int i = 0; i < LINES; i++)
	{
		...
		else
		{
			if ((lline[i] != NULL))
			{
				...
				if (i == 0)
					len += u8g2_GetUTF8Width(&u8g2, nameNum);
				if (len >= x)
				{
					iline[i]++;
					//Max
		...
		for (int i = 0; i < LINES; i++)
		{
			...
			if (lline[i] != NULL)
			{
				...
				if (i == 0)
				{
					...
					u8g2_DrawUTF8(&u8g2, 0, zz + z, lline[i] + iline[i]);
			}
			vTaskDelay(1);
		}
		screenBottomU8g2();
...
void drawTimeU8g2(uint8_t mTscreen, unsigned timein)
{
	...
	{
		if (getDdmm())
			sprintf(strdate, "%02hu-%02hu-%04d", dt->tm_mday, dt->tm_mon + 1, dt->tm_year + 1900);
		else
			sprintf(strdate, "%02hu-%02hu-%04d", dt->tm_mon + 1, dt->tm_mday, dt->tm_year + 1900);
...
void lcd_initU8g2(uint8_t *lcd_type)
{
	...
	else //BW I2C
	{
		...
		if ((sda == GPIO_NONE) || (scl == GPIO_NONE))
		{
			*lcd_type = LCD_NONE;
			return;
		}
	...
	if (*lcd_type != LCD_NONE)
	{
		...
		do
		{
			...
			if (yy >= logo_height)
				...
				u8g2_DrawXBM(&u8g2, x / 2 - logo_width / 2, 0, logo_width, logo_height, logo_bits);
		} while (u8g2_NextPage(&u8g2));
		// u8g2_SendBuffer(&u8g2);
