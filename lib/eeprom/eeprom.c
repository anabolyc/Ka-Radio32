...
void eeEraseStations()
{
	...
	if (buffer != NULL)
	{
		...
		ESP_LOGE(TAG, "Warning %s malloc low memory", "eeEraseStations");
}

void saveStation(struct shoutcast_info *station, uint16_t position)
{
	uint32_t i = 0;
	if (position > NBSTATIONS - 1)
	{
...
{
	uint32_t i = 0;
	while ((position + number - 1) > NBSTATIONS - 1)
	{
...
struct shoutcast_info *getStation(uint8_t position)
{
	if (position > NBSTATIONS - 1)
	{
		kprintf("getStation fails pos=%d\n", position);
		return NULL;
	}
...
void saveDeviceSettings(struct device_settings *settings)
{
	if (settings == NULL)
	{
		ESP_LOGE(TAG, "saveDeviceSetting fails");
		return;
	}
