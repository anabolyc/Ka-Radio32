...
void ramInit()
{
	...
	else
	{
		...
		{
			if (getSPIRAMSIZE() == SMALLRAM * 1024)
				return;						// no need
			setSPIRAMSIZE(SMALLRAM * 1024); // more free heap
		}
		else
		{
			if (getSPIRAMSIZE() == DEFAULTRAM * 1024)
				return;						  // no need
...
void ramSinit()
{
	...
	if (https && !bigSram())
	{
		if (get_audio_output_mode() == VS1053)
		{
			if (getSPIRAMSIZE() == HTTPSVSRAM * 1024)
				return; // no need
			setSPIRAMSIZE(HTTPSVSRAM * 1024);
		}
		else
		{
			if (getSPIRAMSIZE() == HTTPSRAM * 1024)
				return; // no need
...
bool clientParsePlaylist(char *s)
{
	...
	if (str != NULL)
	{
		...
		if (str[i] == '/') // path
		{
			...
			{
				while ((str[i] != 0x0a) && (str[i] != 0x0d) && (str[i] != 0) && (str[i] != '"') && (str[i] != '<') && (j < PATHMAX))
				{
					path[j] = str[i];
					i++;
					j++;
...
static void wsHeaders()
{
	...
	if (wsh == NULL)
	{
		ESP_LOGV(TAG, strcMALLOC1, "wsHeader");
		return;
	}
